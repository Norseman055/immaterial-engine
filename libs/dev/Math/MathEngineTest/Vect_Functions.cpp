...
TEST( Normalize, vector_tests )
{
	...
	CHECK( eq(v1[x],0.267f,MATH_TOLERANCE) );
	CHECK( eq(v1[y],0.534f,MATH_TOLERANCE) );
	CHECK( eq(v1[z],.80174f,MATH_TOLERANCE) );
	CHECK( (v1[w] == 1.0f) );
}
...
TEST( GetNormalize, vector_tests )
{
	...
	CHECK( eq(v2[x],0.267f,MATH_TOLERANCE) );
	CHECK( eq(v2[y],0.534f,MATH_TOLERANCE) );
...
TEST( Mag, VectFunctionTests )
{
	...
	CHECK( eq(value,3.741f,MATH_TOLERANCE));
}
